...
# for (i in 1:n) {
#   # sum of (x_i^2)/2
#   sigma_hat_mle <- sigma_hat_mle + ((wind_data$x[i]^2)/2)
# }
# # 1/n * (sum of (x_i^2)/2)
# sigma_hat_mle <- sigma_hat_mle/n
# # (1/n * (sum of (x_i^2)/2))^(0.5)
# sigma_hat_mle <- sigma_hat_mle^(0.5)

# x <- rexp(n, rate = 1/sqrt_inverse_i)
# hist(x)
# quantile(x, c(.025, .975))

# 
# for (i in 1:m) {
#   # For each iteration, simulate n = 1000 values of X
#   ### where x follows a rayleigh(sigma)
#   X <- rrayleigh(n, scale = sigma_hat_mle)
#   # Find the mean of x, and you get you Y'_i value
#   Y[i] <- mean(X)
#   if (sd(X) > sd_ori) {
#     P_SD_Y[i] <- 1
#   }
#   new_ci = quantile(X, c(0.025, 0.975))
#   new_ci_u = mean(X)+1.96*(sd(X)/sqrt(n))
#   X_L_CI <- rrayleigh(n, scale = ci[1])
#   # if (sd(X_L_CI) < ci[1]) {
#   if (sd(X_L_CI) > sd_ori) {
#     P_CI_L[i] <- 1
#   }
#   X_U_CI <- rrayleigh(n, scale = ci[2])
#   if (sd(X_U_CI) > sd_ori) {
#     P_CI_U[i] <- 1
#   }
#   # difference = sd(X_U_CI) - sd(X_L_CI)
#   # if (difference > ci_range) {
#   #   P_CI_U[i] <- 1
#   # }
# }
# get_prob_performance <- function(results){
#   hist(result)
# }
# hist(Y, main = "Histogram of Average wind speeds, Next 1000 days",
#      xlab = "Average Wind Speeds (MPH)")
# summary(Y)
# sd(Y)
# IQR(Y)
